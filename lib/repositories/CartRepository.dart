...
Future<Stream<Cart>> getCart() async {
  ...
  print(_apiToken);
  final String url =
      '${GlobalConfiguration().getValue('api_base_url')}carts?${_apiToken}with=food;food.restaurant;food.offer;extras&search=user_id:${_user.id}&searchFields=user_id:=';
  print("this is cart details $url");
...
Future<Cart> addCart(Cart cart, bool reset) async {
  ...
  print("this is add to cart $url");
  final client = new http.Client();
  print("this is add to cart body ${json.encode(cart.toMap())}");
  final response = await client.post(
    url,
    headers: {HttpHeaders.contentTypeHeader: 'application/json'},
    body: json.encode(cart.toMap()),
  );
  print("this is add to cart response ${response.body}");
  try {
...
Future<Cart> updateCart(Cart cart) async {
  ...
  final client = new http.Client();
  print("this is update cart $url");
  final response = await client.put(
    url,
    headers: {HttpHeaders.contentTypeHeader: 'application/json'},
    body: json.encode(cart.toMap()),
  );
  print("this is update cart body ${json.encode(cart.toMap())}");
  print("this is update cart response body ${response.body}");
...
Future<bool> removeCart(Cart cart) async {
  ...
  print("this is remove cart $url");
  print("this is remove cart body ${json.encode(cart.toMap())}");
  print("this is remove cart response body ${response.body}");
